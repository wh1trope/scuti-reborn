...
public class InvokeDynamicTransformer extends Transformer {
    ...
    public void transform() {
        this.getClasses().values().stream()
                ...
                .forEach(classNode -> {
                    ...
                    if (this.insertDynamic(classNode, bootstrapName, decryptValue)) {
    ...
    private boolean insertDynamic(final ClassNode classNode, final String bootstrapName,
        ...
        classNode.methods.stream().filter(methodNode -> !Modifier.isAbstract(methodNode.access)).forEach(methodNode -> Arrays.stream(methodNode.instructions.toArray())
